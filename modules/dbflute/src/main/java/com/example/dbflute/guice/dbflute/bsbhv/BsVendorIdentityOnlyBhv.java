...
public abstract class BsVendorIdentityOnlyBhv extends AbstractBehaviorWritable<VendorIdentityOnly, VendorIdentityOnlyCB> {
     ...
     * VendorIdentityOnlyCB cb = new VendorIdentityOnlyCB();
     * cb.query().setFoo...(value);
     * OptionalEntity&lt;VendorIdentityOnly&gt; entity = vendorIdentityOnlyBhv.<span style="color: #DD4747">selectEntity</span>(cb);
     *
     * <span style="color: #3F7E5E">// if the data always exists as your business rule</span>
     * entity.<span style="color: #DD4747">required</span>(vendorIdentityOnly -&gt; {
     *     ...
     * });
     * VendorIdentityOnly vendorIdentityOnly = entity.entity.<span style="color: #DD4747">get()</span>;
     *
     ...
     * <pre>
     * vendorIdentityOnlyBhv.<span style="color: #DD4747">selectScalar</span>(Date.class).max(new ScalarQuery() {
     ...
     */
    public void load(List<VendorIdentityOnly> vendorIdentityOnlyList, ReferrerLoaderHandler<LoaderOfVendorIdentityOnly> handler) {
        xassLRArg(vendorIdentityOnlyList, handler);
        handler.handle(new LoaderOfVendorIdentityOnly().ready(vendorIdentityOnlyList, _behaviorSelector));
    }
     ...
     * @param handler The callback to handle the referrer loader for actually loading referrer. (NotNull)
     */
    public void load(VendorIdentityOnly vendorIdentityOnly, ReferrerLoaderHandler<LoaderOfVendorIdentityOnly> handler) {
        xassLRArg(vendorIdentityOnly, handler);
        handler.handle(new LoaderOfVendorIdentityOnly().ready(xnewLRAryLs(vendorIdentityOnly), _behaviorSelector));
    }
     ...
     */
    public OutsideSqlAllFacadeExecutor<VendorIdentityOnlyBhv> outsideSql() {
        return doOutsideSql();
    }
